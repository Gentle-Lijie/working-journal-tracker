...
@router.get("/daily")
def daily_stats(date: Optional[str] = Query(None)):
    """每日统计"""
    if date:
    ...
    with get_db_session() as session:
        ...
        sessions = (
            ...
            .filter(WorkSession.start_time >= start, WorkSession.start_time <= end)
            .all()
        )
        ...
        journals = (
            ...
            .filter(JournalEntry.start_time >= start, JournalEntry.end_time <= end)
            .all()
        )
...
@router.get("/tokens")
def token_stats(days: int = Query(7, le=90)):
    """Token使用统计"""
...
def work_type_stats(days: int = Query(30, le=365)):
    """工作类型分布统计"""
    start = datetime.now() - timedelta(days=days)

    with get_db_session() as session:
        journals = session.query(JournalEntry).filter(JournalEntry.created_at >= start).all()
